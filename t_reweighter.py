...
for i in range(2, 11):
    ...
    if i <= 5:
        ...
        binning_st = [300000, 350000, 400000, 450000, 500000, 550000, 600000, 650000, 700000, 750000, 800000, 850000, 900000, 1000000, 1200000, 2000000, 3000000]
    else:
        binning_st = [400000, 600000, 800000, 1000000, 1200000, 1400000, 1800000, 2200000, 2600000, 3000000]

    # rwt_ht = TTbarInclPlot(rwt, "HT", "weight", (3000, 0, 3000000), array.array('d', binning_st))
    # drawStack(rwt_ht, "H_{T} [MeV]", regionTeX, f"plots/{i}jets/stack_ht_fr_os" + suffix_before)

    rwt_st = TTbarInclPlot(rwt, "ST", "weight", (3000, 0, 3000000), array.array('d', binning_st))
